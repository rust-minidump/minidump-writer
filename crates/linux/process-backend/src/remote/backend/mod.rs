...
impl<T: Transport> crate::Backend for Backend<T> {
...
impl<T: Transport> Drop for Backend<T> {
    fn drop(&mut self) {
...
#[derive(Debug)]
pub struct ProcessReader<'a, T: Transport> {
    backend: &'a Backend<T>,
}

impl<'a, T: Transport> crate::ProcessReader for ProcessReader<'a, T> {
...
#[derive(Debug)]
pub struct FileReader<'a, T: Transport> {
    backend: &'a Backend<T>,
    handle: usize,
}

impl<'a, T: Transport> crate::FileReader for FileReader<'a, T> {
...
impl<'a, T: Transport> Drop for FileReader<'a, T> {
    fn drop(&mut self) {
        if let Err(e) = self
            .backend
...
#[derive(Debug)]
pub struct DirReader<'a, T: Transport> {
    backend: &'a Backend<T>,
    handle: usize,
}

impl<'a, T: Transport> crate::DirReader for DirReader<'a, T> {
...
impl<'a, T: Transport> Drop for DirReader<'a, T> {
    fn drop(&mut self) {
        if let Err(e) = self
            .backend
...
#[derive(Debug)]
pub struct MappedModuleMemoryReader<'a, T: Transport> {
    backend: &'a Backend<T>,
    handle: usize,
}

impl<'a, T: Transport> crate::MappedModuleMemoryReader for MappedModuleMemoryReader<'a, T> {
...
impl<'a, T: Transport> Drop for MappedModuleMemoryReader<'a, T> {
    fn drop(&mut self) {
        if let Err(e) = self
            .backend
