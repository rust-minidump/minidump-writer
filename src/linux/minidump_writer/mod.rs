...
use {
    super::{
        ...
        dso_debug::{elf_addr_to_usize, write_dso_debug_stream},
...
impl MinidumpWriter {
    ...
    fn write_dump(
        ...
        let dirent =
            match write_dso_debug_stream(self.process_inspector.as_ref(), buffer, &self.auxv) {
                Ok(dirent) => dirent,
                Err(e) => {
    ...
    fn modules_via_debugger_rendezvous(
        ...
        let Some(program_header_count) =
            self.auxv.get_program_header_count().map(elf_addr_to_usize)
        else {
